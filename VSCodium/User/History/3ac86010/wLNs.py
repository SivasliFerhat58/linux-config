...
async def teslim(ctx):
    ...
    if ctx.message.attachments:
        ...
        if user_data["chapter"] >= len(chapterdata["chapters"])+1:
            await ctx.send("Tüm Görevler Tamamlandı. Yeni Görevler Eklenene Kadar Bekleyin.")
            return
...
#Todo                next_channel = discord.utils.get(channels, name=f"chapter_{user_data['chapter']}")
            ...
            if user_data["lesson"]+1 > len(chapterdata["chapters"][user_data["chapter"]-1]["lessons"]):
            ...
            user_data[f"chapter-{new_chapter}_lesson-{new_lesson}"] = puan
